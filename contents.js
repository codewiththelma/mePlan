...
const calendarListEl = document.getElementById("contentListView");
...
async function loadAllEvents() {
  const snapshot = await getDocs(collection(db, "users", auth.currentUser.uid, "contents"));
...
function renderListView() {
  ...
  if (!monthEvents.length) {
    calendarListEl.innerHTML =
      `<p class="empty-state card">No contents planned for this month yet</p>`;
    return;
  }
  ...
  sortedEvents.forEach(item => {
    ...
    card.innerHTML = `
  <div class="content-accent" style="background:${item.color || '#a855f7'}"></div>
  ...
  <div class="content-card-body">
    ...
    ${item.platform
  ? `<p class="content-sub"><strong>Platform:</strong> ${item.platform}</p>`
  : ""}

${item.desc
  ? `<p class="content-desc"><strong>Note:</strong> ${item.desc}</p>`
  : ""}
  </div>
`;


    card.onclick = () => openEditEventModal(item);
...
function openEditEventModal(evt) {
  ...
  eventPlatformInput.value = evt.platform;
  eventDescInput.value = evt.desc;
...
eventForm.addEventListener("submit", async (e) => {
  ...
  const platform = eventPlatformInput.value.trim();
  ...
  if (id) {
    await updateDoc(doc(db, "users", auth.currentUser.uid, "contents", id), { title, date, color });
    showSuccess("Event updated!");
  } else {
    await addDoc(collection(db, "users", auth.currentUser.uid, "contents"), {
      title,
      platform,
      desc,
      date,
...
confirmDelete.addEventListener("click", async () => {
  ...
  await deleteDoc(doc(db, "users", auth.currentUser.uid, "contents", pendingDeleteId));
